...
export function parseRunFile(json: unknown): RunFile {
  ...
  const requiredStrings = ['build_id', 'game_mode', 'killed_by_encounter', 'killed_by_event', 'platform_type', 'seed']
  ...
  const requiredNumbers = ['ascension', 'run_time', 'schema_version', 'start_time']
  ...
  if (!hasArray(obj, 'acts') || !hasArray(obj, 'map_point_history') || !hasArray(obj, 'players')) {
    throw new ParseError('Missing or invalid array field: acts / map_point_history / players')
  }

  const schemaVersion = obj.schema_version as number
  if (!SUPPORTED_RUN_SCHEMA_VERSIONS.has(schemaVersion)) {
    console.warn(`[STS2 Parser] Unsupported run schema version: ${schemaVersion}. Parsing may be inaccurate.`)
  }
...
export function parseProgressFile(json: unknown): ProgressFile {
  ...
  if (!SUPPORTED_PROGRESS_SCHEMA_VERSIONS.has(schemaVersion)) {
    console.warn(`[STS2 Parser] Unsupported progress schema version: ${schemaVersion}`)
  }
...
export async function batchParseRunFiles(files: File[]): Promise<{ data: RunFile, file: File }[]> {
...
export async function scanDirectoryHandle(dirHandle: FileSystemDirectoryHandle): Promise<File[]> {
  const runFiles: File[] = []

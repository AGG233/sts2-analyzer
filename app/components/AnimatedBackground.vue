...
const loadSpineBackground = async () => {
  ...
  const { SkeletonBinary, TextureAtlas, AtlasAttachmentLoader } = spineCoreModule
  ...
  const loadSpineLayer = async (dir: string, name: string, scale: number) => {
    ...
    const skeletonData = skelBinary.readSkeletonData(new Uint8Array(skelBuffer))
  ...
  const positionLogo = (spine: any, scale: number) => {
    const bounds = spine.getLocalBounds()
    if (bounds.width <= 0 || bounds.height <= 0) return
  ...
  resizeObserver = new ResizeObserver(() => {
    if (app) {
      ...
      stage.children.forEach((child: any, index: number) => {
        const layer = layers[index]
        if (!layer) return
        if (layer.dir === 'logo') {
          positionLogo(child, layer.scale)
        } else {
          coverSpine(child)
        }
      })

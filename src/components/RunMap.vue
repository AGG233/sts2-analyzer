...
const emit = defineEmits<{
  (e: 'selectFloor', floor: number): void
}>()
...
const svgHeight = computed(() => {
  ...
  const contentHeight = topPadding + (totalNodes * verticalSpacing) + (actGaps * 1.5 * verticalSpacing) + bottomPadding

  // 额外空间：确保底层节点可以滚到底部
  const extraPadding = containerHeight.value > 0 ? containerHeight.value - verticalSpacing : 0
  // 如果内容高度超过容器高度，使用内容高度（允许滚动）
  // 否则使用容器高度（填满空间）
  return containerHeight.value > 0 ? Math.max(contentHeight + extraPadding, containerHeight.value) : contentHeight
})

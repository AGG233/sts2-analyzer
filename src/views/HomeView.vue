  ...
  <div class="home">
    ...
    <div v-if="characterStats.length > 0" class="character-stats">
      ...
      <DataTable :value="characterStats" sort-mode="single" removable-sort>
        <Column field="character" :header="t('ui.home.character')" sortable>
          <template #body="{ data }">
            {{ characterName(data.character) }}
          </template>
        </Column>
        <Column field="wins" :header="t('ui.home.wins')" sortable>
          <template #body="{ data }">
            <Tag :value="String(data.wins)" severity="success" />
          </template>
        </Column>
        <Column field="losses" :header="t('ui.home.losses')" sortable>
          <template #body="{ data }">
            <Tag :value="String(data.losses)" severity="danger" />
          </template>
        </Column>
        <Column field="total" :header="t('ui.home.total')" sortable />
        <Column field="winRate" :header="t('ui.home.winRate')" sortable>
          <template #body="{ data }">
